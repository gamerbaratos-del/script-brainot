...
local modifiedParts = {} -- [part] = originalCanCollide

-- Ensure collision group exists and is non-collidable with Default
local function ensureCollisionGroup()
	-- Try to find existing groups
	local groups = {}
	local ok, result = pcall(function() return PhysicsService:GetCollisionGroups() end)
	...
	for _, g in ipairs(groups) do
		-- API returns tables with name field
	...
	-- Make the group not collide with Default and with itself to be safe
	pcall(function()
...
local function setCharacterNoCollide(character)
	...
	for _, desc in ipairs(character:GetDescendants()) do
		if desc:IsA("BasePart") then
			...
			pcall(function()
				PhysicsService:SetPartCollisionGroup(desc, COLLISION_GROUP)
			end)
		end
	end

	-- Monitor new parts (e.g. accessories added later)
...
local function restoreCharacterCollisions()
	...
	-- Restore parts we modified
	for part, original in pairs(modifiedParts) do
		if part and part.Parent then
			-- restore original CanCollide (safe fallback to true if nil)
...
local function enableFly()
	...
	-- Make character non-collidable while flying
	local character = player.Character
	if character then
		setCharacterNoCollide(character)
	end
...
player.CharacterAdded:Connect(function()
	-- Clear state on respawn
	flying = false
	bodyVel = nil
	bodyGyro = nil

	-- ensure collisions are restored (in case)
	restoreCharacterCollisions()
